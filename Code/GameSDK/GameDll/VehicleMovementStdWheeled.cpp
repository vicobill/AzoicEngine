...
void CVehicleMovementStdWheeled::ApplyBoost(float speed, float maxSpeed, float strength, float deltaTime)
{
  ...
  {     
    ...
    const float amount = fraction * strength * m_action.pedal * m_PhysDyn.mass * deltaTime;
  
    const float angle = DEG2RAD(m_carParams.steerTrackNeutralTurn == 0.f ? 30.f : 0.f);
    const Vec3 dir(0, cosf(angle), -sinf(angle));

    const Matrix33 worldMat(m_PhysPos.q);
    Vec3 boostOrigin = ZERO;

    if (IVehicleHelper* pBoostOriginHelper = m_pVehicle->GetHelper("boost_origin"))
    {
      boostOrigin = pBoostOriginHelper->GetWorldSpaceTranslation();
    }
    else
    {
      // Fallback to origin implementation
      CryWarning(VALIDATOR_MODULE_GAME, VALIDATOR_COMMENT, "Vehicle: Boosted a vehicle without a boost origin helper");

      AABB bounds;
      m_pVehicle->GetEntity()->GetLocalBounds(bounds);
      boostOrigin = worldMat * Vec3(0.f, bounds.min.y, 0.f) + m_PhysPos.pos;
    }

    pe_action_impulse imp;            
	imp.impulse = worldMat * dir * amount;
    imp.point = boostOrigin;
    imp.iApplyTime = 0;
