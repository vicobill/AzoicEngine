	...
	if (dir>0) {
		...
		for(int i=0; i*sizeof(void*) < stmBuf.m_iPos; i++) {
			...
			switch (pent->GetType()) {
				...
				pe_status_awake temp;
				int awake = pent->GetStatus(&temp);
				pe_action_awake aa;
				pent->Action(&aa,1);
...
void CPhysRenderer::DrawTextBuffers(HDC hDC)
{
	...
	m_textQ.iterate([&](STextRec& txt)->bool {
		...
		if (pt.y>0) {
			Vec3 temp = Vec3(*(Vec3_tpl<uchar>*) & txt.color) * (1.0f / 256);
			glColor3fv((float*)&temp);
			float ry = k/pt.y;
...
#define _clr(c) { auto temp = c; glColor4ubv((GLubyte*)&temp); }
#define _vtx(v) { auto temp = v; glVertex3fv((const GLfloat*)&temp); }
