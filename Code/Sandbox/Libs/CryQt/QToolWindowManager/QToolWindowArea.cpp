...
bool QToolWindowArea::eventFilter(QObject* pObject, QEvent* pEvent)
{
	if (pObject == tabBar() || pObject == cornerWidget() || pObject == m_pTabFrame || pObject == m_pTabFrame->m_pCaption)
	{
		...
		{
			QMouseEvent* me = static_cast<QMouseEvent*>(pEvent);
			if (m_tabDragCanStart && (me->pos() - m_mousePressPosition).manhattanLength() > QApplication::startDragDistance())
			{
			...
			else if (m_areaDragCanStart && (me->pos() - m_mousePressPosition).manhattanLength() > QApplication::startDragDistance())
			{
...
bool QToolWindowArea::event(QEvent* event)
{
	...
	{
		QMouseEvent* me = static_cast<QMouseEvent*>(event);
		if (m_areaDragCanStart && (me->pos() - m_mousePressPosition).manhattanLength() > QApplication::startDragDistance())
		{
