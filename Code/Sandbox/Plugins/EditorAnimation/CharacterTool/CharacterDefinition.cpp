...
void CharacterAttachment::SerializeVCloth(Serialization::IArchive& ar)
{
	if (ar.openBlock("vcloth-parameter", "+Parameter"))
	{
		...
		if (ar.openBlock("simulation", "+Simulation and Collision"))
		{
			...
			ar(Serialization::Range(m_vclothParams.airResistance, 0.0f, 10000.0f), "airResistance", "Air Resistance");
			ar.doc("If >0, sets how strongly the cloth is affected by environmental wind. For example: use 5 for light-to-moderate strength.");
		...
		if (strcmp(tag, "AttachmentList") == 0)
		{
			...
			for (uint32 i = 0; i < num; i++)
			{
				...
				if (Type == "CA_VCLOTH")
				{
					...
					nodeAttach->getAttr("airResistance", attach.m_vclothParams.airResistance);
...
void CharacterDefinition::ExportVClothAttachment(const CharacterAttachment& attach, XmlNodeRef nodeAttachments)
{
	...
	nodeAttach->setAttr("airResistance", attach.m_vclothParams.airResistance);
