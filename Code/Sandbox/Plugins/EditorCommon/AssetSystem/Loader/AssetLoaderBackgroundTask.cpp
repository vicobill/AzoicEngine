...
{
public:
	static void RunAsync(std::vector<CAsset*>&& assets)
	{
...
public:
	static void RunAsync(const std::vector<string>& assetRootPaths, FinalizeFunc finalize)
	{
	...
	{
		CAssetLoaderBackgroundTask task(assetRootPaths, std::move(finalize));
		task.Work();
		if (task.m_finalize)
		{
			task.m_finalize(task.m_assets);
		}
	}
	...
	virtual void Finalize() override
	{
		...
		CSaveCacheBackgroundTask::RunAsync(std::move(m_assets));
	}
private:
...
void RunAsync(const std::vector<string>& assetRootPaths, std::function<void(const std::vector<CAsset*>&)> finalize)
{
	...
	CAssetLoaderBackgroundTask::RunAsync(assetRootPaths, std::move(finalize));
}
