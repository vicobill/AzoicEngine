...
std::future<std::invoke_result_t<std::decay_t<Fn>>> Async(Fn&& fn, Args&&... args)
{
...
template<typename Fn, typename... Args>
std::future<std::invoke_result_t<std::decay_t<Fn>>> AsyncQueue(Fn&& fn, Args&&... args)
{
	using namespace Private_ThreadingUtils;
...
template<typename Fn, typename... Args>
std::future<std::invoke_result_t<std::decay_t<Fn>, std::decay_t<Args>...>> PostOnMainThread(Fn&& fn, Args&&... args)
{
	using namespace Private_ThreadingUtils;
...
std::future<std::invoke_result_t<std::decay_t<WorkFn>>> AsyncFinalize(WorkFn&& workFn, FinalizeFn&& finalizeFn)
{
	return Async([workFn, finalizeFn]() -> std::invoke_result_t<std::decay_t<WorkFn>>
	{
		auto&& res = workFn();
...
//! \sa AsyncFinalize
template<typename WorkFn, typename FinalizeFn>
std::future<std::invoke_result_t<std::decay_t<WorkFn>>> AsyncNotify(WorkFn&& workFn, FinalizeFn&& finalizeFn)
{
	return Async([workFn, finalizeFn]() -> std::invoke_result_t<std::decay_t<WorkFn>>
	{
		auto&& ret = workFn();
