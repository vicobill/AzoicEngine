...
CSharedFlashPlayerResources::CSharedFlashPlayerResources()
	...
	, m_pSys_scaleform_shared_font_tex_size(nullptr)
	...
	#endif
{
	...
	m_pSys_scaleform_shared_font_tex_size = REGISTER_INT("sys_scaleform_shared_font_tex_size", 1024, VF_REQUIRE_APP_RESTART, "Additional flash/scaleform font texture raster size for both width and height.");

	gEnv->pLog->Log("Using Scaleform GFx " GFC_FX_VERSION_STRING);
	m_pGSystemInit = new GSystemInitWrapper();
	assert(m_pGSystemInit);
	m_pLoader = new GFxLoader2(m_pSys_scaleform_shared_font_tex_size->GetIVal(), m_pSys_scaleform_shared_font_tex_size->GetIVal());
...
CSharedFlashPlayerResources::~CSharedFlashPlayerResources()
{
	...
	gEnv->pConsole->UnregisterVariable("sys_scaleform_shared_font_tex_size");
}
...
GFxLoader2::GFxLoader2(int textureWidth, int textureHeight)
	...
	, m_parserVerbosity()
{
	...
	SetupDynamicFontCache(textureWidth, textureHeight);
...
void GFxLoader2::SetupDynamicFontCache(int textureWidth, int textureHeight)
{
	SetFontPackParams(0);
