...
void CScriptGraphFunctionNode::CreateInputsAndOutputs(CScriptGraphNodeLayout& layout, const IEnvFunction& envFunction)
{
	...
	{
		if (const CCommonTypeDesc* pArrayValueDesc = envFunction.GetInputArrayType(inputIdx))
		{
			const CryGUID typeGUID = pArrayValueDesc->GetGUID();
			layout.AddInput(CUniqueId::FromUInt32(envFunction.GetInputId(inputIdx)), envFunction.GetInputName(inputIdx), typeGUID, { EScriptGraphPortFlags::Data, EScriptGraphPortFlags::Array });
		}
		else
		{
