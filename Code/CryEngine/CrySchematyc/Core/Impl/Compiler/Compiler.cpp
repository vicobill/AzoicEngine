...
bool CCompiler::CompileClass(const IScriptClass& scriptClass)
{
	...
	if (pEnvClass)
	{
		...
		// Check and warn about any orphaned script functions
		for (auto pFunc : pClass->GetFunctions())
		{
			if (pFunc.graphIdx == InvalidIdx)
			{
				IScriptRegistry& scriptRegistry = gEnv->pSchematyc->GetScriptRegistry();
				IScriptElement* pElement = scriptRegistry.GetElement(pFunc.guid);

				const stack_string errorMsg = stack_string().Format("Orphaned script function detected '%s'. Look for errors or bad nodes in this graph: %s:%s.", pFunc.guid.ToString().c_str(), pClass->GetName(), pElement ? pElement->GetName() : "Unknown");

				CRY_ASSERT(false, errorMsg);
