...
bool CObject::CreateComponents()
{
	...
	if (IEntity* pEntity = GetEntity())
	{
		...
		pEntity->VisitComponents([](IEntityComponent* pComponent) { pComponent->Initialize(); });
	}
...
void CObject::ShutdownComponents()
{
	if (IEntity* pEntity = GetEntity())
	{
		pEntity->VisitComponents([&pEntity](IEntityComponent* pComponent) { pEntity->RemoveComponent(pComponent); });
	}
}
