...
#if !_RELEASE
		if (!m_stack.empty())
			Warning("Warning: Profiler scopes didn't match in the previous frame (added N section labels, but only removed N-%d)\n", m_stack.size());
#endif
...
#if !_RELEASE
		if (!m_stack.empty())
			Warning("Warning: Profiler scopes didn't match in the current frame (added N section labels, but only removed N-%d)\n", m_stack.size());
#endif
