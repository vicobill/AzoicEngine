...
public:
	...
	void           Execute(CTexture* pSrcRT, CTexture* pDestRT, const RECT* pSrcRect = NULL, const RECT* pDstRect = NULL, bool bBigDownsample = false, const ColorF& color = ColorF(1, 1, 1, 1), const uint64 renderStateFlags = 0);
	bool           PreparePrimitive(CRenderPrimitive& prim, CPrimitiveRenderPass& targetPass, const RECT& rcS, uint64 renderState, const D3DViewPort& targetViewport, bool bResample, bool bBigDownsample, CTexture* pSrcRT, CTexture* pDstRT, const ColorF& color);
...
protected:
	bool PreparePrimitive(CRenderPrimitive& prim, uint64 renderState, int stencilState, const ColorF& cClear, float cDepth, int stencilRef, const RECT& rect, const D3DViewPort& targetViewport, int numRTVs);
