...
struct SSF_GlobalDrawParams
{
	...
	uint64       blendModeStates;
	uint64       renderMaskedStates;
	...
	struct OutputParams : private NoCopy
	{
		...
		mutable CClearRegionPass clearColorPass;
		mutable CClearRegionPass clearDepthStencilPass;

		// Handling clears
		mutable bool bRenderTargetClear;
		mutable bool bStencilTargetClear;

		OutputParams(CGraphicsPipeline* pGraphicsPipeline) : clearColorPass(pGraphicsPipeline), clearDepthStencilPass(pGraphicsPipeline) {}
	};
