...
inline uint64 SF_AdjustBlendStateForMeasureOverdraw(uint64 blendModeStates)
{
	...
	, m_pShader(0)
{
	...
	m_pShader = gRenDev->m_cEF.mfForName("Scaleform3", EF_SYSTEM);
}
...
{
	if (!CRY_VERIFY(m_pSFResD3D != nullptr))
	{
		// Should not enter here. But create resources to avoid undefined behavior.
		SF_CreateResources();
	}
...
void CD3D9Renderer::SF_HandleClear(const SSF_GlobalDrawParams& __restrict params)
{
	...
	if (rCurOutput.bRenderTargetClear || rCurOutput.bStencilTargetClear)
	{
		...
		if (rCurOutput.bRenderTargetClear)
			rCurOutput.clearColorPass.Execute(rCurOutput.pRenderTarget, Clr_Transparent, 1, &rect);
		if (rCurOutput.bStencilTargetClear)
			rCurOutput.clearDepthStencilPass.Execute(rCurOutput.pStencilTarget, CLEAR_STENCIL, Clr_Unused.r, Val_Stencil, 1, &rect);
	}
...
void CD3D9Renderer::SF_DrawIndexedTriList(int baseVertexIndex, int minVertexIndex, int numVertices, int startIndex, int triangleCount, const SSF_GlobalDrawParams& __restrict params)
{
...

	{
		...
		primInit->SetTechnique(sfRes.m_pShader, techName, 0);
