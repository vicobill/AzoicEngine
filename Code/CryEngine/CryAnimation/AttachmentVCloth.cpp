...
void CClothSimulator::PositionsIntegrate()
{
	...
	const float kwind = m_config.airResistance * sqr(m_dt) / m_dtNormalize;
	static Vec3 g_wind(10,0,0);
	Vec3 wind(ZERO), windNorm;
	bool useWind = false;
	Matrix33 tmp;
	if (m_config.airResistance > 0)
	{
		pe_params_buoyancy pb[4];
		if (--m_windCheckTimer <= 0)
		{
			m_wind.zero();
			for(int i = gEnv->pPhysicalWorld->CheckAreas(m_location.t, m_gravity, pb, 4) - 1; i >= 0; i--)
			{
				if (pb[i].iMedium == pe_params_buoyancy::eAir) m_wind += pb[i].waterFlow;
			}
		...
		if ((useWind = (wind = m_wind / m_dtNormalize).len2()))
		{
			...
			// normal is the normal of the best-fitting plane running through those vertices
			// windage (convexity or concavity wrt the wind direction) is computed based on that plane's origin
			m_vtxNeighbCount.resize(m_nVtx);
			m_vtxNeighbCenter.resize(m_nVtx);
			m_vtxNeighbCmtx.resize(m_nVtx);
			memset(m_vtxNeighbCount.data(), 0, m_nVtx*sizeof(int));
			memset(m_vtxNeighbCenter.data(), 0, m_nVtx*sizeof(Vec3));
			memset(m_vtxNeighbCmtx.data(), 0, m_nVtx*sizeof(Matrix33));
			for(int i=0; i<m_nEdges;i++) 
			{
				const int i1 = m_links[i].i1, i2 = m_links[i].i2;
				Vec3 edge = Vec3(m_particlesHot[i2].pos - m_particlesHot[i1].pos).GetNormalizedFast();
				m_vtxNeighbCount[i1]++; m_vtxNeighbCount[i2]++;
				m_vtxNeighbCenter[i1] += edge; m_vtxNeighbCenter[i2] -= edge;
				m_vtxNeighbCmtx[i1] += dotproduct_matrix(edge, edge, tmp);
				m_vtxNeighbCmtx[i2] += tmp;
			}
		}
	}
	...
	for (int i = 0; i < m_nVtx; i++)
	{
		...
		if (useWind && m_vtxNeighbCount[i] > 2)
		{
			// C is used to solve for the best fitting plane normal
			// plane's origin is the mean of the sampling points
			Matrix33 C = m_vtxNeighbCmtx[i];
			Vec3 org = m_vtxNeighbCenter[i] / static_cast<float>(m_vtxNeighbCount[i]);
			C -= dotproduct_matrix(org,org,tmp) * float(m_vtxNeighbCount[i]);
			std::tuple<float, int> maxdet = std::tuple<float, int>{ 0.f, -1 };
			for(int j = 0; j < 3; j++) 
			{
				float det = C(incm3(j),incm3(j)) * C(decm3(j),decm3(j)) - C(decm3(j),incm3(j)) * C(incm3(j),decm3(j));
				if (fabs(det) > fabs(std::get<0>(maxdet))) maxdet = std::tuple<float, int>{ det, j };
			}
			if (std::get<1>(maxdet) >= 0) 
			{
				float rdet = 1.0f / std::get<0>(maxdet);
				Vec3 n;	int j = std::get<1>(maxdet);
				n[j] = 1;
				n[incm3(j)] = -(C(incm3(j),j) * C(decm3(j),decm3(j)) - C(decm3(j),j) * C(incm3(j),decm3(j))) * rdet;
				n[decm3(j)] = -(C(incm3(j),incm3(j)) * C(decm3(j),j) - C(decm3(j),incm3(j)) * C(incm3(j),j)) * rdet;
				// wind is applied along the normal, scaled by normal*wind_vector and windage 
				// windage is 0..2; 0 = fully convex towards the wind (0 degrees peak); 1 = flat; 2 = fully concave (0 degrees hole)
				Vec3 windLoc = n * ((wind - dv) * n) * (1 - (n * org) * sgnnz(n * wind));
				m_particlesHot[i].pos += windLoc * kwind;
			}
		}
	}
}
