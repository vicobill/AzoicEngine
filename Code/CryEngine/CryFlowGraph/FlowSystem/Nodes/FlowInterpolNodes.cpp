...
float CubicInOut(float ratio)
{
	...
	else
	{
		...
		return (ratio - 1.0f) * pow(factor, 2.f) + 1.0f;
	}
}

float QuartIn(float ratio)
{
	return pow(ratio, 4.f);
}
...
float BounceOut(float ratio)
{
	...
	const float amplitude = pow(length, 2.f);
