...
{
	static CryGFxTranslator* s_pTranslatorInst(new CryGFxTranslator(WWT_Default));
	if (createNew)
	{
...
Image* CryImageCreator::LoadProtocolImage(const Scaleform::GFx::ImageCreateInfo& info, const Scaleform::String& url)
{
	...
	const char* pTexId(strstr(url, "://TEXID:"));
	if (pTexId)
	{
		if (IScaleformRecording* pRenderer = CSharedFlashPlayerResources::GetAccess().GetRenderer(true))
		{
			...
			CRY_ASSERT(pTextureManager);

			pTexId += 9;
			int texId(atoi(pTexId));

			if (ITexture* pTexture = gEnv->pRenderer->EF_GetTextureByID(texId))
			{
				if (GPtr<Scaleform::Render::Texture> pSFtexture = pTextureManager->CreateTexture(static_cast<CTexture*>(pTexture)))
				{
