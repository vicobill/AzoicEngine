...
public:
	CCaptureDrawStats(IScaleformRecording* pRenderer, SFlashProfilerData* pProfilerData, bool capture)
		...
		, m_capture(capture)
	{
		CRY_ASSERT(m_pRenderer != nullptr);
		IF (m_capture, 0)
		{
...
public:
	CSyncSection(GFxMovieView* pMovieView, CryCriticalSectionPtr pSection)
		: m_pSection(pSection), m_pMovieView(pMovieView)
	{
		m_pSection->Lock();
		if (m_pMovieView)
		{
			m_pMovieView->SetCaptureThread(Scaleform::GetCurrentThreadId());
		}
	}
...
static inline SFlashVarValue ConvertValue(const GFxValue& src)
{
	...
	case GFxValue::VT_Int:
		return SFlashVarValue(static_cast<int>(src.GetInt()));

	case GFxValue::VT_UInt:
		return SFlashVarValue(static_cast<uint32>(src.GetUInt()));
...
void CFlashVariableObject::VisitMembers(ObjectVisitor* pVisitor) const
{
	...
	if (m_value.IsObject() && pVisitor)
	{
		struct VisitorAdaptor : public GFxValue::ObjectVisitor
		{
			...
			{
				CRY_ASSERT(m_pClientVisitor != nullptr);
...
bool CFlashVariableObject::Set3DMatrix(const Matrix44& mat)
{
	...
	if (m_value.IsDisplayObject())
	{
		CRY_ASSERT(fabsf(mat.Determinant()) > 1e-4f);
		GMatrix3D m;
		m.M[0][0] = mat.m00;
		m.M[0][1] = mat.m10;
		m.M[0][2] = mat.m20;
...
{
	CRY_ASSERT(m_pLoader.GetPtr() != nullptr);
...
bool CFlashPlayerBootStrapper::Load(const char* pFilePath)
{
	...
	if (m_pMovieDef)
	{
		if (isVideoFile)
		{
			GPtr<GFxFileOpener> pFileOpener = *new CryGFxResourceOpener(videoPath);
			m_pMovieDef->SetFileOpener(pFileOpener);
		}
		else if (m_pMovieDef->GetFileOpener().GetPtr() != &CryGFxFileOpener::GetAccess())
		{
			m_pMovieDef->SetFileOpener(&CryGFxFileOpener::GetAccess());
		}
	}
#endif // USE_GFX_VIDEO
...
CFlashPlayer::CFlashPlayer()
	...
	#endif
{
	CRY_ASSERT(m_lock.get() != nullptr);
	CRY_ASSERT(m_filePath.get() != nullptr);
...
CFlashPlayer::~CFlashPlayer()
{
	...
	GPtr<Scaleform::Render::CSFRenderCache> pRenderCache = m_pRenderCache;
	GPtr<GFxMovieDef> pMovieDef = m_pMovieDef;
	GPtr<GFxMovieView> pMovieView = m_pMovieView;
	uint32 memArenaID = m_memArenaID;
	...
	#endif
		if (m_pMovieView)
			m_pMovieView->SetActionControl(nullptr);
	...
	CRY_ASSERT(!pMovieView || 1 == pMovieView->GetRefCount());
	CD3D9Renderer* pRenderer = static_cast<CD3D9Renderer*>(gEnv->pRenderer);
	auto rtDestruction = [=]
	{
		if (pMovieView)
			pMovieView->SetCaptureThread(Scaleform::GetCurrentThreadId());

		const_cast<GPtr<GFxMovieView>&>(pMovieView).Clear();
		const_cast<GPtr<GFxMovieDef>&>(pMovieDef).Clear();
		const_cast<GPtr<Scaleform::Render::CSFRenderCache>&>(pRenderCache).Clear();
		CSharedFlashPlayerResources::GetAccess().DestoryMemoryArena(memArenaID);
	};
	pRenderCache = 0;
	pMovieView = 0;
	pMovieDef = 0;

	pRenderer->ExecuteRenderThreadCommand(std::move(rtDestruction));
...
bool CFlashPlayer::ConstructInternal(const char* pFilePath, GFxMovieDef* pMovieDef, unsigned int options, unsigned int cat)
{
	...
	if (pMovieDef->GetFileOpener().GetPtr() != &CryGFxFileOpener::GetAccess())
	{
		isVideoFile = IsVideoFile(pFilePath);
		if (isVideoFile)
		{
			options &= ~INIT_FIRST_FRAME;
		}
	}
#endif
	...
	// additionally assigning handlers to definition so that callbacks on construction can be handled (or ignored) and don't cause warnings
	pMovieDef->SetFSCommandHandler(&CryGFxFSCommandHandler::GetAccess());
	pMovieDef->SetExternalInterface(&CryGFxExternalInterface::GetAccess());
	pMovieDef->SetUserEventHandler(&CryGFxUserEventHandler::GetAccess());
	...
	if (isVideoFile)
	{
		...
		if (GFxVideoPlayer::LoadVideoInfo(pFilePath, &videoInfo, m_pLoader->GetFileOpener()))
		{
			if (IsStereoVideoFile(pFilePath))
				videoInfo.Width = (videoInfo.Width << 1) / 3;

			*const_cast<int*>(&m_width) = videoInfo.Width;
			*const_cast<int*>(&m_height) = videoInfo.Height;
			m_pMovieView->SetVariable("_global.gfxArg", pFilePath);
		}
		else
		{
...
bool CFlashPlayer::Bootstrap(GFxMovieDef* pMovieDef, unsigned int options, unsigned int cat)
{
	...
	if (pMovieDef->GetFileOpener().GetPtr() != &CryGFxFileOpener::GetAccess())
	{
		pFilePath = ((CryGFxResourceOpener*)pMovieDef->GetFileOpener().GetPtr())->GetResourcePath();
	}
#endif // USE_GFX_VIDEO
...
void CFlashPlayer::Advance(float deltaTime)
{
	...
	if (m_pMovieView)
	{
		...
		// Passing false to "capture" means the frame capturing is not happening here, but we call Capture() function explicitly in RenderToTexture function.
		// Capturing here could produce frames that might not be consumed if the RenderToTexture is not called (e.g. the Flash element is hidden, but we still want to Advance it)
		m_pMovieView->Advance(deltaTime, 2, false);
	}
}
...
Scaleform::Render::RenderTarget* CFlashPlayer::CreateRenderTarget(CTexture* pTexture)
{
	...
	CRY_ASSERT(pRenderOutput != nullptr);
	if (!pRenderOutput)
		return nullptr;

	CSFRenderer* pSFRenderer = ((CSFRenderer*)m_pRenderer.GetPtr());
	CTexture* pRenderTarget = nullptr;
	CTexture* pStencilTarget = nullptr;

	if (auto* pDC = pRenderOutput->GetDisplayContext())
	{
		pRenderTarget = pDC->GetCurrentColorOutput();
		pStencilTarget = pDC->GetCurrentDepthOutput();
	}
	else
	{
		pRenderTarget = pRenderOutput->GetColorTarget();
		pStencilTarget = pRenderOutput->GetDepthTarget();
	}

	if (pRenderTarget)
		return pSFRenderer->CreateRenderTarget(pRenderTarget, pStencilTarget);
...
void CFlashPlayer::RenderToTexture(ITexture* pTexture)
{
	...
	if (pSFTarget)
	{
        ...
        if (hMovieDisplay.NextCapture(pSFRenderer->GetContextNotify()))
        {
			if (m_clearFlags)
			{
				CSFRenderTargets* pSFRenderTarget = (CSFRenderTargets*)pSFTarget->GetRenderTargetData();
				Scaleform::Render::Color bgColor = hMovieDisplay.GetRenderEntry()->GetBackgroundColor();

				bool bRenderTargetClear = pSFRenderTarget->pRenderTargetTexture && (m_clearFlags & FRT_CLEAR_COLOR);
				bool bStencilTargetClear = pSFRenderTarget->pDepthStencilTexture && (m_clearFlags & FRT_CLEAR_STENCIL);

				if (255 > bgColor.GetAlpha() && bRenderTargetClear)
				{
					CClearSurfacePass& clearPassRT = m_pRenderCache->GetClearPass();
					clearPassRT.Execute(pSFRenderTarget->pRenderTargetTexture, m_clearColor);
				}
				if (bStencilTargetClear)
				{
					CClearSurfacePass& clearPassST = m_pRenderCache->GetClearPass();
					clearPassST.Execute(pSFRenderTarget->pDepthStencilTexture, CLEAR_STENCIL, Clr_Unused.r, Val_Stencil);
				}
			}
...
void CFlashPlayer::RenderPlaybackLocklessCallback(int cbIdx, EFrameType ft, bool finalPlayback)
{
	...
	CRY_ASSERT(false, "CFlashPlayer::RenderPlaybackLocklessCallback() -- Not implemented!");
}
...
struct FunctionHandlerAdaptor : public GFxFunctionHandler
{
	...
	FunctionHandlerAdaptor(IActionScriptFunction* pFunc, CFlashPlayer* pPlayer)
		...
		, m_pPlayer(pPlayer)
	{
		CRY_ASSERT(m_pFunc != nullptr);
		CRY_ASSERT(m_pPlayer != nullptr);
	}
	...
	void Call(const GFxFunctionHandler::Params& params)
	{
		...
		CRY_ASSERT(pTranslatedArgsWithThisRef != nullptr);

		CRY_ASSERT(params.pArgsWithThisRef != nullptr);
		...
		if (numArgs)
		{
			...
			CRY_ASSERT(ppTranslatedArgs != nullptr);
			for (unsigned int i = 0; i < numArgs; ++i)
		...
		// call function
		CRY_ASSERT(params.pMovie != nullptr);
...
void CFlashPlayer::DelegateExternalInterfaceCallback(const char* pMethodName, const GFxValue* pArgs, UInt numArgs)
{
	...
	if (m_pMovieView)
	{
		...
		if (m_pEIHandler)
		{
			CRY_ASSERT(pArgs || !numArgs); // We should allow 0 arguments
...
{
	#if defined(ENABLE_FLASH_INFO)
	CryAutoCriticalSection theLock(GetList().GetLock());

	IRenderer* pRenderer = gEnv->pRenderer;
	CRY_ASSERT(pRenderer != nullptr);
...
void CFlashPlayer::DumpAndFixLeaks()
{
	...
	{
		CryAutoCriticalSection theLock(CFlashVariableObject::GetList().GetLock());
	...
	{
		CryAutoCriticalSection theLock(CFlashPlayerBootStrapper::GetList().GetLock());
