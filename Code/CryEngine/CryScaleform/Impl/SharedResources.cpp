...
CSharedFlashPlayerResources::~CSharedFlashPlayerResources()
{
	...
	assert(!m_pImageCreator || m_pImageCreator->GetRefCount() == 1);
	SAFE_RELEASE(m_pImageCreator);
	assert(!m_pImageFileRegistry || m_pImageFileRegistry->GetRefCount() == 1);
	SAFE_RELEASE(m_pImageFileRegistry);
