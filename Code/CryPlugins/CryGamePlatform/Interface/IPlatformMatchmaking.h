	...
	namespace GamePlatform
	{
		...
		struct IMatchmaking
		{
			...
			struct IListener
			{
				...
				//! Called when the OS requests the game to invite other players
				virtual void OnExternalInvitationRequest(const DynArray<AccountIdentifier>& invitees) = 0;
				//! Called when a player attempts to join a lobby. If at least one listener returns true, the process is aborted
				virtual bool OnPreJoinLobby(const LobbyIdentifier& lobbyId) = 0;
			};
			...
			virtual bool CreateLobby(IUserLobby::EVisibility visibility, int maxMemberCount) = 0;
