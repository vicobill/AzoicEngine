...
namespace Cry
{
	namespace GamePlatform
	{
		namespace Steam
		{
			...
			void CLeaderboards::DownloadEntries(const LeaderboardIdentifier& leaderboardId, ERequest request, int minRange, int maxRange, bool bTimeBased)
			{
				...
				FindOrCreateLeaderboard(ExtractLeaderboardName(leaderboardId), sortMethod, displayType);
			}
			...
			void CLeaderboards::OnEntriesDownloaded(LeaderboardScoresDownloaded_t* pResult, bool bIOFailure)
			{
				...
				for (int i = 0; i < pResult->m_cEntryCount; i++)
				{
					...
					{
						LeaderboardIdentifier leaderboardIdentifier(SteamServiceID, LeaderboardIdentifierValue(pSteamUserStats->GetLeaderboardName(pResult->m_hSteamLeaderboard)));
						...
						const EScoreType scoreType = static_cast<EScoreType>(details[0]);
						const char* szPersonaName = pSteamFriends->GetFriendPersonaName(leaderboardEntry.m_steamIDUser);

						m_listeners.ForEach([isCurrentUser, &leaderboardEntry, leaderboardIdentifier, scoreType, szPersonaName](IListener* pListener) {
							pListener->OnEntryDownloaded(leaderboardIdentifier, szPersonaName, leaderboardEntry.m_nGlobalRank, leaderboardEntry.m_nScore, scoreType, isCurrentUser);
						});
					}
				}
			}

			void CLeaderboards::UpdateScore(const LeaderboardIdentifier& leaderboardId, int score, EScoreType scoreType, bool bForce)
			{
				if (SteamUserStats() == nullptr)
				{
					return;
				...
				request.name = ExtractLeaderboardName(leaderboardId);
