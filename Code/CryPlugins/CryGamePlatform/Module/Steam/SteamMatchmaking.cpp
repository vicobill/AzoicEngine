...
namespace Cry
{
	namespace GamePlatform
	{
		namespace Steam
		{
			...
			bool CMatchmaking::CreateLobby(IUserLobby::EVisibility visibility, int maxMemberCount)
			{
			...
			{
				bool abortJoinLobby = false;
				m_listeners.ForEach([&abortJoinLobby, &lobbyId](IListener* pListener) {
					if (!abortJoinLobby)
						abortJoinLobby = pListener->OnPreJoinLobby(lobbyId);
				});
				if (abortJoinLobby)
				{
					CryLog("[Steam] JoinLobby() aborted due to OnPreJoinLobby returning true");
					return;
				}
			...
			void CMatchmaking::OnCreateLobby(LobbyCreated_t* pResult, bool bIOFailure)
			{
				if (pResult->m_eResult == k_EResultOK)
				{
					...
					m_listeners.ForEach([pLobby](IListener* pListener) {
			...
			{
				m_listeners.ForEach([pLobbyMatchList](IListener* pListener) {
			...
			void CMatchmaking::OnJoin(LobbyEnter_t* pCallback, bool bIOFailure)
			{
				...
				else
				{
					...
					m_listeners.ForEach([&lobbyId](IListener* pListener) {
						pListener->OnLobbyJoinFailed(lobbyId);
					});
				}
			}
