		...
		namespace Steam
		{
			...
			void CUserGeneratedContentManager::OnContentCreated(CreateItemResult_t* pResult, bool bIOError)
			{
				if (pResult->m_eResult == k_EResultOK)
				{
					...
					{
						stack_string url = stack_string().Format("steam://url/CommunityFilePage/%" PRIu64, pResult->m_nPublishedFileId);
						m_service.OpenBrowser(url);
					}
					...
					m_listeners.ForEach([pContent, pResult](IListener* pListener) {
