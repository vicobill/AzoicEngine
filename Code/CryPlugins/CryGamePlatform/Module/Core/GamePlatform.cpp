...
namespace Cry
{
	namespace GamePlatform
	{
		...
		void CPlugin::MainUpdate(float frameTime)
		{
			...
			if (m_services.size() != m_loggedInServices)
			{
				size_t loggedInServices = FindNumberOfLoggedInServices();
				if (m_loggedInServices != loggedInServices)
				{
					AddAccounts();
				}
			}
			...
			{
				EnableUpdate(Cry::IEnginePlugin::EUpdateStep::MainUpdate, false);
			}
		}

		void CPlugin::AddAccounts() const
		{
			size_t previousLoggedInServices = m_loggedInServices;
			m_loggedInServices = 0;
			...
			if (previousLoggedInServices != m_loggedInServices)
			{
				m_users[0] = stl::make_unique<CUser>(userAccounts);
			}
		}

		size_t CPlugin::FindNumberOfLoggedInServices() const
		{
			size_t loggedServices = 0;
			for (IService* const pService : m_services)
			{
				if (pService != nullptr)
					loggedServices += pService->IsLoggedIn() ? 1 : 0;
			}
		...
		IStreamInstall* CPlugin::GetStreamInstall() const
		{
			if (const IService* pMainService = GetMainService())
			{
				return pMainService->GetStreamInstall();
			}
		...
		void CPlugin::RegisterMainService(IService& service)
		{
			...
			if (GetMainService() == nullptr)
			{
				...
				m_services[0]->AddListener(*this, "GamePlatform");
			}
		}
		...
		void CPlugin::RegisterService(IService& service)
		{
			...
			{
				service.AddListener(*this, "GamePlatform");
			}
		}
