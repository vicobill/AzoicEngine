...
namespace UQS
{
	namespace Core
	{
		...
		{
			return m_functionBinding.inputs.size();
		}

		uint32 CSchematycUqsComponentEnvFunctionBase::GetInputId(uint32 inputIdx) const
		{
			return inputIdx < m_functionBinding.inputs.size() ? m_functionBinding.params.at(m_functionBinding.inputs.at(inputIdx)).id : Schematyc::InvalidId;
		}

		const char* CSchematycUqsComponentEnvFunctionBase::GetInputName(uint32 inputIdx) const
		{
			return inputIdx < m_functionBinding.inputs.size() ? m_functionBinding.params.at(m_functionBinding.inputs.at(inputIdx)).name.c_str() : "";
		}
		...
		{
			return inputIdx < m_functionBinding.inputs.size() ? m_functionBinding.params.at(m_functionBinding.inputs.at(inputIdx)).pData.get() : nullptr;
		}

		uint32 CSchematycUqsComponentEnvFunctionBase::GetOutputCount() const
		{
			return m_functionBinding.outputs.size();
		}

		uint32 CSchematycUqsComponentEnvFunctionBase::GetOutputId(uint32 outputIdx) const
		{
			return outputIdx < m_functionBinding.outputs.size() ? m_functionBinding.params.at(m_functionBinding.outputs.at(outputIdx)).id : Schematyc::InvalidId;
		}

		const char* CSchematycUqsComponentEnvFunctionBase::GetOutputName(uint32 outputIdx) const
		{
			return outputIdx < m_functionBinding.outputs.size() ? m_functionBinding.params.at(m_functionBinding.outputs.at(outputIdx)).name.c_str() : "";
		}
		...
		{
			return outputIdx < m_functionBinding.outputs.size() ? m_functionBinding.params.at(m_functionBinding.outputs.at(outputIdx)).pData.get() : nullptr;
		}
		...
		bool CSchematycUqsComponentEnvFunctionBase::AreAllParamtersRecognized() const
		{
			...
			for (uint32 i = 0; i < m_functionBinding.params.size(); ++i)
			{
				if (!m_functionBinding.params.at(i).pData)
					return false;
			}
			return true;
		}
		...
		{
			m_functionBinding.inputs.push_back(m_functionBinding.params.size());
			m_functionBinding.params.push_back(CreateParam(szParamName, id, pTypeName, pTypeDesc, Schematyc::EnvFunctionUtils::EParamFlags::BoundToInput));
		}
		...
		{
			m_functionBinding.outputs.push_back(m_functionBinding.params.size());
			m_functionBinding.params.push_back(CreateParam(szParamName, id, pTypeName, pTypeDesc, Schematyc::EnvFunctionUtils::EParamFlags::BoundToOutput));
		}
		...
		{
			CRY_ASSERT(inputIdx < m_functionBinding.inputs.size());

			const uint32 paramIdx = m_functionBinding.inputs.at(inputIdx);
			const SMyParamBinding& paramBinding = m_functionBinding.params.at(paramIdx);
		...
		{
			CRY_ASSERT(outputIdx < m_functionBinding.outputs.size());

			const uint32 paramIdx = m_functionBinding.outputs.at(outputIdx);
			const SMyParamBinding& paramBinding = m_functionBinding.params.at(paramIdx);
			const Schematyc::CUniqueId id = Schematyc::CUniqueId::FromUInt32(paramBinding.id);
		...
		CSchematycUqsComponentEnvFunctionBase::SMyParamBinding CSchematycUqsComponentEnvFunctionBase::CreateParam(const char* szParamName, uint32 id, const Schematyc::CTypeName* pTypeName, const Schematyc::CCommonTypeDesc* pTypeDesc, Schematyc::EnvFunctionUtils::EParamFlags paramFlags) const
		{
			...
			p.flags = paramFlags;
			p.idx = m_functionBinding.params.size();
			p.id = id;
			p.name = szParamName;
